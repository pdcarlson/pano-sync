...
export const renameImageFiles = async (imageFiles, prefix) => {
  const renamedFiles = imageFiles.map((file) => {
    ...
    // if the file doesn't match the expected pano format, we can choose to ignore it or handle it differently.
...
export const mergeJsonData = (existingJson, newJson) => {
  // simply spread both objects; newJson keys will overwrite existing ones if they conflict.
